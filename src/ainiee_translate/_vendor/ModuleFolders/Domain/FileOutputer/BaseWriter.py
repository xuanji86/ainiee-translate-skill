...
class BaseTranslatedWriter(BaseTranslationWriter):
    ...
    def pre_write_translated(self, translation_file_path: Path, cache_file: CacheFile) -> PreWriteMetadata:
        ...
        # 决定使用的编码
        # [ainiee-translate patch] always UTF-8 output (drops the WriterUtil /
        # TaskConfig / PyQt config dependency; matches our default config)
        keep_original_encoding_config = False
